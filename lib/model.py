...
def getGenre():
    ...
    if page.status_code == 200:
        ...
        # GETTING DATA
        for data in soup.find('ul' , attrs={'class' , 'genre'}).find_all('li'):
            
            if '&nbsp;' not in data.get_text():
                container.append({
                 
                    'title': str(data.get_text()).split('=')[-1].strip(),
					'id' : data.find('a').get('href').replace('https://komikcast.com/genres/' , '')[:-1] if data.find('a') is not None else None,
                })
...
def getKomikGenre():
    ...
    order = '' if req.args.get('order') is None else '?order=' + req.args.get('order')
    idKomik = req.args.get('id')
    newUrl = urlPath + 'genres' + idKomik + order if req.args.get('page') is None else urlPath + 'daftar-komik/page/' + req.args.get('page') + '/' + order
    pagination_page = int(req.args.get('page')) if req.args.get('page') is not None else 1
    page = requests.get(newUrl , headers=headers)
    soup = bs(page.text , 'html.parser')
